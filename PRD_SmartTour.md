...
- **In scope:** QR gate, bản đồ POI, audio đa ngôn ngữ, offline map/audio, analytics, device presence, premium MoMo, **CMS Geofence Simulator + ILogRunner (append/đọc file log)**.
...
- **Lượt ghé POI (visit analytics):** gửi `POST /api/analytics/visit` với `VisitType` — **Geofence** (đi kèm heatmap zone_enter/app_open), **MapClick** (chọn POI trên bản đồ), **QRCode** (QR cổng hợp lệ dạng `smarttour://poi/{id}`). Backend enqueue, worker ghi batch vào `visit_logs`.
...
- `FoodTranslation`
- `PlayLog`
- `visit_logs` (entity `VisitLog`: lượt ghé POI theo `VisitType`, ghi theo lô từ worker)
- `HeatmapEntry`
- `RouteSession`
- `RouteSessionPoi`
...
| `VisitLog` / `visit_logs` | Ghi lượt ghé POI (Geofence / MapClick / QRCode) | Không sửa | Không xóa thủ công | System | Ingest qua channel + worker batch |
...
- **Payload:** `poiId`, `lat`, `lng`, `visitType` (`Geofence` | `MapClick` | `QRCode`), `userId` tùy chọn khi ẩn danh.
...
- **CMS (Admin — minh chứng / load test):** **Geofence Simulator** gửi visit qua `LoadTest/FireGeofenceVisit` tới cùng endpoint trên; `userId` bắt buộc tiền tố `SIM-` (máy ảo `SIM-DEV-xx`). Đọc kết quả batch qua `LoadTest/RecentVisitLogs`. **Không** thay thế logic app MAUI; phục vụ demo và kiểm thử hàng đợi + DB. Chi tiết URL: mục [21.1](#211-geofence-simulator-cms--url-và-hai-luồng-log).
...
- **Load test/Index & GeofenceSimulator:** `Index` có **Bulk visit (stress)** (song song `api/analytics/visit`, `SIM-BULK-*`); `GeofenceSimulator` mô phỏng geofence đa thiết bị, proxy visit, poll `visit_logs`, lưu client log qua `ILogRunner`; panel log audio **full width** dưới bản đồ, xem file log bằng **modal** (xem mục 4.2, 16.9, 21.1).
...
- `POST /api/analytics/visit` (**AllowAnonymous** + rate limit `DeviceTokenPolicy`; **202 Accepted** `{ message: "Visit queued" }` sau enqueue; `poiId <= 0` → 400; queue đầy / kênh đóng → 503)
...
- [ ] `POST /api/analytics/visit` trả 202 và bản ghi xuất hiện trên DB sau worker (Geofence / MapClick / QRCode).
...
| Shared Models | `SmartTour.Shared/SmartTour.Shared/Models/*.cs` (gồm `VisitType`, `VisitLog`, `NarrationTelemetry` / `NarrationTelemetryBus`) |
...
| Visit từ app | `SmartTourApp/Services/ApiService.cs` (`PostVisitAsync`), `HeatmapService.cs`, `MapPage.xaml.cs`, `QrGatePage.xaml.cs` |
