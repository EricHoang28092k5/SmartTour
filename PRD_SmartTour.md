...
| **Trạng thái** | Đồng bộ với mã nguồn repo: **ví vendor** (`vendor_wallets` / nạp MoMo + trừ ví), **Premium CMS** (Admin áp dụng trực tiếp; Vendor mua bằng ví), **`VisitLog.SpeedKmh`**, giá gói **`MoMo:PackagePrice`**; Geofence + Bulk visit stress + log file như 7.7 |
...
1. Vendor/Admin tạo POI trên CMS (Vendor: đủ số dư ví cho **`PoiCreation:FixedVendorCreateChargeVnd`** trước khi xác nhận tạo).
