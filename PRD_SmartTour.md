...
sequenceDiagram
    ...
    participant API as SmartTourAPI
    participant DB as PostgreSQL
    U->>CMS: Mở Dashboard — HomeController.Index
    CMS->>API: GET /api/cms-dashboard/device-status — fetch (cookie)
    API->>DB: Query DevicePresences + tính IsActive theo LastSeenUtc — HomeController.GetDeviceStatus
    API-->>CMS: Online count + danh sách thiết bị — Ok(JSON)
    loop mỗi 3 giây
      CMS->>API: Refresh device-status — setInterval → fetch
      API-->>CMS: Dữ liệu mới — Ok(JSON)
    end
```
...
sequenceDiagram
    ...
    CMS->>API: POST /api/vendor/premium/purchase-premium-wallet-cms (+ X-Internal-Key) — HttpClient PostAsync
    ...
    API-->>CMS: success + balanceVnd — Ok(result)
...
sequenceDiagram
    ...
    API->>API: Validate payload + threshold nghe — validation in action
    ...
    alt duplicate_window_15s hoặc queue_full
      ING-->>API: accepted=false + reason — tuple result
      ...
      W->>DB: Bulk insert PoiAudioListenEvents — AddRange + SaveChangesAsync
    end
```
...
```mermaid
flowchart TD
    A[Mở dashboard] -->|LogController.Plays / JS fetch| B[Gọi API thống kê]
    B -->|HomeController.GetDashboardPoiStats| C[Lọc dữ liệu theo role]
    C -->|GroupBy PlayLog| D[Vẽ chart/table]
    D -->|View render| E[Kết thúc]
```
